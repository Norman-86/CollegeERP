<?php 
    ...
    $total_records = $pages['num'];
        ...
        // Get page data
        $query1 = "SELECT f.feeID,f.amount,f.registered,fa.name AS faculty,c.name as course,y.yos,s.sem, p.start,p.end
                   FROM fee f
                    LEFT JOIN course c
                    ON f.courseID = c.courseID
                    LEFT JOIN faculty fa
                    ON c.facultyID = fa.facultyID
                    LEFT JOIN yos y 
                    ON f.yosID = y.yosID
                    LEFT JOIN semister s
                    ON f.semisterID = s.semisterID
...
<head>
    ...
    <title>View Fee | Admissions | WAKA CMEC</title>
...
<body style="background-color:rgba(42,63,84,0);">
    ...
    <div class="row" style="width:98%;margin-right:auto;margin-left:auto;padding-top:10px;-webkit-box-shadow:0px 4px 4px 0px rgba(0,0,0,1);-moz-box-shadow:0px 4px 4px 0px rgba(0,0,0,1);box-shadow:0px 4px 4px 0px rgba(0,0,0,1);margin-bottom:5%;overflow:hidden;">
        <form method="POST">
            ...
            ?>
            <div class="col-md-12">
                <h2 style="font-weight:normal;">View Fee</h2>
            </div>
                            ...
                            <tbody>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td style="font-weight:normal;font-size:12px;color:rgb(6,6,6);"> <?php echo $row['faculty'];?> </td>
                                        ...
                                        <td style="font-size:12px;font-weight:normal;color:rgb(3,3,3);"> <?php echo "Kshs.".number_format($row['amount'],2);?> </td>
                                        <td style="font-size:12px;font-weight:normal;color:rgb(3,3,3);"> 
                                            <?php echo date_format(date_create($row['registered']),"d M, Y.").date(" g:i A", strtotime($row['registered']));?> 
                                        </td>
                                    </tr>
                                <?php } ?>
                            </tbody>
