<?php 
    ...
    if(isset($_GET['id'])){
        ...
        // Get page data
        $quer = "SELECT * FROM unit WHERE unitID='$unitID'";
...
<head>
    ...
    <title>Update Unit | Administrator | WAKA CMEC</title>
    ...
    <div class="row" style="width:100%;margin-right:auto;margin-left:auto;-webkit-box-shadow:0px 4px 4px 0px rgba(0,0,0,1);-moz-box-shadow:0px 4px 4px 0px rgba(0,0,0,1);box-shadow:0px 4px 4px 0px rgba(0,0,0,1);margin-bottom:5%;overflow:hidden;max-width:960px;">
        ...
        <form method="POST">
            <?php 
                ...
                {
                while ($name1 = mysql_fetch_array($results)) {
            ?>
            <div style="padding-top:2px;padding-bottom:2px;background-color:rgb(239,239,239);">
                <h2 style="margin-left:1%;margin-bottom:1%;font-size:16px;color:rgb(1,1,1);font-weight:bold;"><i class="fa fa-refresh"></i> &nbsp;Update Unit</h2>
            </div>
            <div class="row" style="padding:10px;">
                <div class="col-md-12">
                    <?php include 'validate/updateUnit.php'; ?>
                    <div class="row">
                        <div class="col-md-12 col-sm-12">
                            <div class="form-group">
                                <label class="control-label">Unit: &nbsp;<a class="text-danger">*</a></label>
                                <input class="form-control" type="text" value ="<?php echo $name1['name'];?>" placeholder="Unit Name" name="unit" style="text-transform: capitalize;">
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        ...
                        <div class="col-md-12 col-sm-12">
                            <div class="form-group">
                                ...
                                <select class="form-control" id="yos" name="yos">
                                    ...
                                    <?php
                                        if($name1['courseID']){
                                            $sql = "select years from `course` where `courseID`=".$name1['courseID'];
                                            $ress = mysql_query($sql);
                                            if(mysql_num_rows($ress) > 0) {
                                                while($rw = mysql_fetch_array($ress)){
                                                  $sq = mysql_query("select * from yos where yos <=".$rw['years']);
                                                    while($raw = mysql_fetch_array($sq)) {
                                    ?>
                                    <option value='<?php echo $raw['yosID']; ?>' <?php if($name1['yosID']==$raw['yosID']){echo "selected='selected'";}?>> <?php echo "Year ".$raw['yos']; ?></option>
                                    <?php
                                                    }
                                                }
                                            } 
                        ...
                        <div class="col-md-12 col-sm-12">
                            <div class="form-group">
                                ...
                                <select class="form-control" name="sem">
                                    ...
                                    <option value="1" <?php if($name1['semisterID']==1){ echo "selected=selected";}?> >Semester 1</option>
                                    <option value="2" <?php if($name1['semisterID']==2){ echo "selected=selected";}?> >Semester 2</option>
                                </select>
                            </div>
                        </div>
                    </div>
